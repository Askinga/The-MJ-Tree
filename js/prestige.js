addLayer("p", {
	...
	doReset(p) {
        ...
        let keep = [];
	    if (hasMilestone('l', 0)) keep.push("upgrades");
    ...
    hotkeys: [
        {key: "p", description: "P: Reset for prestige points (Uni. 1)", onPress(){if (canReset(this.layer) && !(inChallenge('universes', 11))) doReset(this.layer)}},
    ],
...
if (player.p.autoRC.gt(0) && hasMilestone('s', 0) && player.p.runeChoose.gt(0)) {
	...
	player.p.autoRC = ARC
		...
		gain = gain.times(tmp.b.bad)
		gain = gain.times(tmp.b.good)
