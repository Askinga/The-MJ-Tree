	...
	tabFormat: {
		...
		"QoL Tree": {
			content: [
				...
				["display-text", function(){ if (hasUpgrade('tm', 91)) return "You have " + format(player.tm.STMP) + " Super True Meta Points, boosting True Meta Runes and Points by x" + format(tmp.tm.STMP) + " (+" + format(tmp.tm.MTMP) + ")<br>You have a " + format(tmp.tm.STMPch.times(100)) + "% chance of getting a Super True Meta Point on reset.<br>"}],
