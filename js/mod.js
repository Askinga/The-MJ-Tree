...
function getPointGen() {
	...
	if (hasUpgrade('p',14)) gain = gain.times(1.75)
	...
	if (hasUpgrade('s', 24)) gain = gain.times(3)
	if (hasUpgrade('s', 32)) gain = gain.times(6)
	if (hasUpgrade('s', 32)) gain = gain.pow(0.95)
	if (hasUpgrade('m', 11)) gain = gain.times(5)
	gain = gain.times(layers.m.effect())
	if (hasUpgrade('m', 21)) gain = gain.times(10)
	if (hasUpgrade('m', 35)) gain = gain.times(upgradeEffect('m', 35))
	if (hasUpgrade('m', 42)) gain = gain.tetrate(upgradeEffect('m', 42))
	gain = gain.pow(1.5)
	return gain
}
