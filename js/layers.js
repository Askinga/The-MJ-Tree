addLayer("p", {
	...
	if (hasUpgrade('m', 13)) mult = mult.times(7.5)
	if (hasUpgrade('m', 32)) mult = mult.times(upgradeEffect('m', 32)) 
	if (hasUpgrade('u', 14)) mult = mult.times(100)
        return mult
    },
	...
	description(){ return "x" + format(upgradeEffect('p',11)) + " Points" },
	effect(){
	    power = new Decimal(1)
    ...
    buyables: {
        11: {
            ...
            display() {
                return "Boost points by ^1.01 per level. (compounding) " + "<br>Cost: " + format(tmp[this.layer].buyables[this.id].cost) + " prestige points" + "<br>Level " + getBuyableAmount(this.layer, this.id) + "/100" + "<br>Effect: Boost Point gain by ^" + format(buyableEffect(this.layer, this.id))
            },
            canAfford() {
