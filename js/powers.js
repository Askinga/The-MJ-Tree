addLayer("pr", {
	...
	onPrestige(){
		...
		if (hasUpgrade('sl', 21)) add = add.add("2^4")
    ...
    exponent() { 
		...
		if (hasUpgrade('sl', 23)) softcap = softcap.times(2)
		if (hasUpgrade('sl', 31)) softcap = softcap.times(upgradeEffect('sl', 31))
	...
	update(diff){
		...
		upg4 = upg4.times(layers.tm.effect())
		...
		if (hasUpgrade('water', 43) || hasUpgrade('tm', 42)) {
