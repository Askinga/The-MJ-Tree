addLayer("water", {
	...
	14: {
        ...
        display() { return "^1.01 Water per purchase<br>Cost: " + format(this.cost()) + " Water<br>Bought: " + format(getBuyableAmount('water', 14)) + "/30<br>Effect: ^" + format(buyableEffect('water', 14)) + " Water" },
        canAfford() { return player.water.points.gte(this.cost()) },
        buy() {
            player.water.points = player.water.points.sub(this.cost())
            setBuyableAmount(this.layer, this.id, getBuyableAmount(this.layer, this.id).add(1))
