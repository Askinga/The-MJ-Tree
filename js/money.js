addLayer("money", {
    ...
    gainMult() { // Calculate the multiplier for main currency from bonuses
		...
		if (hasUpgrade('uni', 11)) mult = mult.times("e20")
        if (hasUpgrade('uni', 31)) mult = mult.times(upgradeEffect('uni', 31))
    ...
    gainExp() { // Calculate the exponent on main currency from bonuses
		...
		if (hasUpgrade('uni', 12)) exp = exp.times(upgradeEffect('uni', 12))
		return exp
    },
	...
	update(diff) {
		...
		if (hasMilestone('uni', 1) || hasUpgrade('tm', 11)) cool = cool.div(30)
